...
3. **Create calendar events using gcalcli:**
   ...
   - For multiple events, chain commands with `&&`:
     ...
     gcalcli add --calendar "YOUR_CALENDAR_NAME" --title "Task 1" --when "09:00" --duration 25 --noprompt && \
     gcalcli add --calendar "YOUR_CALENDAR_NAME" --title "Task 2" --when "09:30" --duration 25 --noprompt && \
     gcalcli add --calendar "YOUR_CALENDAR_NAME" --title "Task 3" --when "10:00" --duration 25 --noprompt
     ```
